...
**How to apply:** Use a plain foreign key to prevent dangling IDs when its deletion semantics are safe. At the API boundary, compare nullable workspace scopes with exact equality so global rows cannot consume tenant-owned IDs. Do not claim database-level tenant enforcement until the global-row policy is represented with NULL-aware constraints or triggers.