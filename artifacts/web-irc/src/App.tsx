...
type ConsoleOverview = {
  ...
  categories: Array<{ id: number; name: string; description: string; communityId: number | null; communityName: string | null }>;
  recentMessages: Array<{ id: string; body: string; sender: string; channelId: number | null; createdAt: string }>;
...
function AdminConsole() {
  ...
  const deleteRegistryResource = async () => {
    if (!pendingResourceDelete || !pendingResourceDelete.item.communityId) return;
    const pending = pendingResourceDelete;
    setWorking(true);
    try {
      if (pending.kind === "channel") {
        const confirmation = `DELETE CHANNEL ${pending.item.name} FROM WORKSPACE ${pending.item.communityName}`;
        await api(`/communities/${pending.item.communityId}/channels/${pending.item.id}`, { method: "DELETE", body: JSON.stringify({ confirmation }) });
        setNotice(`${pending.item.name} and its history were deleted.`);
