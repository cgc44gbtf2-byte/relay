...
type ConsoleOverview = {
  ...
  channels: Array<{ id: number; name: string; topic: string; memberCount: number; communityId: number | null; communityName: string | null; createdAt: string }>;
...
function AdminConsole() {
  ...
  return (
    <div className="irc-grid terminal-sheen min-h-[100dvh] bg-background text-foreground">
      ...
      {pendingResourceDelete && <AdminConfirmDialog title={`Delete ${pendingResourceDelete.item.name}?`} description={pendingResourceDelete.kind === "channel" ? "This permanently deletes the channel, its messages, members, requests, invitations, and stored attachments." : "This deletes the category and moves its channels to Uncategorized. Channel history is preserved."} confirmLabel={pendingResourceDelete.kind === "channel" ? "delete channel" : "delete category"} destructive working={working} onConfirm={() => void deleteRegistryResource()} onCancel={() => setPendingResourceDelete(null)} />}
    </div>
  );
}
