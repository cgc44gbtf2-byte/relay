...
test(
  `preserves provisioning diagnostics in the CI summary (PostgreSQL ${postgresVersion})`,
  async () => {
    const tempDirectory = await mkdtemp(join(tmpdir(), "web-irc-ci-provisioning-"));
    const summaryFile = join(tempDirectory, "summary.md");
    const previousSummary = process.env.GITHUB_STEP_SUMMARY;
    process.env.GITHUB_STEP_SUMMARY = summaryFile;

    try {
      const adminClient = createFakeAdminClient((query) => {
      ...
      await assert.rejects(
        () =>
          runCiTests({
            adminUrl: "postgresql://runner@127.0.0.1:5432/postgres",
            clientFactory: () => adminClient,
            runCommandImpl: async () => {
              throw new Error("test commands should not run after provisioning fails");
            },
          }),
      ...
      assert.match(
        summary,
        /Runner diagnostic: Test database provisioning failed \(PostgreSQL .*?\): permission denied to create database/,
      );
...
test(
  `preserves cleanup diagnostics in the CI summary (PostgreSQL ${postgresVersion})`,
  async () => {
    const tempDirectory = await mkdtemp(join(tmpdir(), "web-irc-ci-cleanup-"));
    const summaryFile = join(tempDirectory, "summary.md");
    const previousSummary = process.env.GITHUB_STEP_SUMMARY;
    process.env.GITHUB_STEP_SUMMARY = summaryFile;
    ...
    try {
      const adminClient = createFakeAdminClient((query) => {
        if (query.startsWith("DROP DATABASE")) {
          throw new Error("permission denied to drop database");
        }
      });
      ...
      const summary = await readFile(summaryFile, "utf8");
      assert.match(summary, /Status: failed/);
      assert.match(summary, /Failing stage: cleanup/);
      assert.match(summary, /Cleanup: failed/);
      assert.match(
        summary,
        /Cleanup diagnostic: Test database cleanup failed \(PostgreSQL .*?\): permission denied to drop database/,
      );
...
function createFakeAdminClient(throwForQuery) {
  return {
    async connect() {},
    async query(query) {
      throwForQuery(query);
