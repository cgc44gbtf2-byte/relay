...
export async function runCiTests({
  ...
  const databaseName = createDatabaseName("current");
  const rehearsalDatabaseName = createDatabaseName("rehearsal");
  ...
  const rehearsalConnectionString = setDatabaseName(
    adminConnectionString,
    rehearsalDatabaseName,
  );

  const adminClient = clientFactory(adminConnectionString);
  ...
  try {
    ...
    stage = "reviewed migration rehearsal";
    try {
      await runCommandImpl(
        "pnpm",
        ["--filter", "@workspace/db", "run", "rehearse:migrations"],
        {
          TEST_DATABASE_URL: rehearsalConnectionString,
        },
      );
    } catch (error) {
      throw createStageError("Reviewed migration rehearsal failed", error);
    }

    stage = "schema prerequisites";
    const schemaClient = clientFactory(testConnectionString);
    try {
      await schemaClient.connect();
      // Reviewed migration 0017 installs this for existing databases; a fresh
      // current-schema push needs it before Drizzle creates trigram indexes.
      await schemaClient.query("CREATE EXTENSION IF NOT EXISTS pg_trgm");
    } catch (error) {
      throw createStageError("Test database schema prerequisites failed", error);
    } finally {
  ...
  try {
    await dropDatabases({
      adminClient,
      adminConnected,
      databaseNames: [databaseName, rehearsalDatabaseName],
    });
...
function createDatabaseName(purpose) {
  const runId = [
  ...
  const availablePrefixLength = 63 - purpose.length - suffix.length - 2;
  return `${prefix.slice(0, availablePrefixLength)}_${purpose}_${suffix}`;
}
...
function createStageError(stage, error) {
  ...
  const versionLabel = ` (PostgreSQL ${process.env.CI_TEST_DATABASE_VERSION || "unspecified"})`;
...
function formatDiagnostic(error) {
  ...
  for (const [name, value] of Object.entries(process.env)) {
    const sensitiveName = /(?:PASSWORD|SECRET|TOKEN|CREDENTIAL|WEBHOOK|PRIVATE|KEY|DATABASE.*URL|POSTGRES.*URL|PG.*URL|AUTH)/i.test(name);
    if (value && value.length >= 4 && (sensitiveName || value.length >= 16)) {
