...
function requireReleaseTarget(environment, databaseUrl) {
...
function requireBackupConfirmation(environment) {
...
export async function runMigrations({
  ...
  requireReleaseTarget(environment, databaseUrl);
  if (mode !== "check") requireBackupConfirmation(environment);
  if (mode === "baseline") {
    if (environment.RELEASE_MIGRATION_BASELINE_CONFIRMED !== "true") {
      throw new Error(
  ...
  try {
    ...
    for (const file of pending) {
      ...
      try {
        ...
        throw new Error(
          `Migration ${file.filename} failed and was rolled back: ${redactConnectionString(errorMessage(error), databaseUrl)}`,
        );
      }
